...
export class ImageService {
  ...
  private async transformContentfulAsset(
    ...
    // Extract URL safely and build final URL with query params
    ...
    const width = imageOptions.width || 
                 (file.details && 
                  ...
                  'width' in file.details.image ? 
                   Number(file.details.image.width) : undefined);
    ...
    const height = imageOptions.height || 
                  (file.details && 
                   ...
                   'height' in file.details.image ? 
                    Number(file.details.image.height) : undefined);
  ...
  async getFallbackImage(type: ImageType, options?: ImageOptions): Promise<BlogImage> {
    ...
    try {
      ...
      console.error('Error getting fallback image:', err);
